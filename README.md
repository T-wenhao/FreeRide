...
[![Hermes Agent Compatible](https://img.shields.io/badge/Hermes_Agent-Compatible-purple.svg)](https://github.com/NousResearch/hermes-agent)
---

**FreeRide** gives you unlimited free AI in [Hermes Agent](https://github.com/NousResearch/hermes-agent) by automatically managing OpenRouter's free models.
...
You're using Hermes Agent. You love it. But:
...
5. **Preserves** your existing Hermes config
...
When you hit a rate limit, Hermes automatically tries the next model. You keep working. No interruptions.
...
# Always restart Hermes after changes:
hermes gateway restart
```
...
## Testing with Your Hermes Agent

After running `freeride auto` and `hermes gateway restart`:
...
1. **Hermes's runtime fallback chain** — when the primary returns 429, the gateway transparently tries fallback 1, 2, 3 at routing time. The agent never sees the 429.
...
**Will it mess up my Hermes config?**

No. FreeRide only touches `model` and `fallback_providers` in `~/.hermes/config.yaml`. Your terminal, auxiliary, delegation, cron, tools, skills - all preserved.

**Which models are free?**

Run `freeride list` to see current availability. It changes, which is why FreeRide exists.

**Do I need to restart Hermes after changes?**

Yes. Run `hermes gateway restart` after any FreeRide command that changes your config.
...
<p align="center">
  ...
  ·
  <a href="https://github.com/NousResearch/hermes-agent">☤ Install Hermes Agent</a>
</p>
