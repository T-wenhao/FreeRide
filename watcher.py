...
Background process that keeps the Hermes model chain healthy without ever
...
def check_and_rotate(state: dict) -> bool:
    ...
    current_api = current.split("/", 1)[-1] if "/" in current else current
    ok, err = _test_model(current_api)
    if ok:
        print(f"[{datetime.now().isoformat()}] {current_api} OK")
        return False

    print(f"[{datetime.now().isoformat()}] {current_api} failed ({err}) — rotating.")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        description="Background process that keeps the Hermes model chain healthy."
    )
