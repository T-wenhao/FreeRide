...
CACHE_FILE = Path.home() / ".hermes" / ".freeride-cache.json"
...
def _parse_env_file() -> list:
    """Parse OPENROUTER_API_KEY from ~/.hermes/.env file."""
    ...
    for line in ENV_PATH.read_text().splitlines():
        ...
        if line.startswith("OPENROUTER_API_KEY="):
            ...
            return _parse_api_keys(val)
    return []


def get_api_keys() -> list:
    """Get all OpenRouter API keys. Checks env var first, then ~/.hermes/.env."""
...
def save_config(config: dict):
    """Save Hermes config to ~/.hermes/config.yaml, preserving key order."""
...
def get_current_model(config: dict = None) -> Optional[str]:
    """Get currently configured model, returns 'provider/model' format."""
    if config is None:
        config = load_config()
...
def get_current_fallbacks(config: dict = None) -> list:
    """Get currently configured fallback models, returns ['provider/model', ...]."""
    if config is None:
        config = load_config()
    result = []
...
def update_model_config(
    ...
    """Update Hermes config with the specified model.

    Args:
        model_id: The OpenRouter model ID (e.g. 'qwen/qwen3-coder:free')
        ...
        append_free: If True, ensure ':free' suffix on non-native model IDs.
    """
    config = load_config()

    # Ensure :free suffix for non-OpenRouter-native models
    if append_free and not model_id.startswith("openrouter/") and ":free" not in model_id:
    ...
    if add_fallbacks and get_api_keys():
        ...
        smart_router_id = "openrouter/free"
        if model_id != smart_router_id:
            fallbacks.append({"provider": "openrouter", "model": smart_router_id})
        ...
        for m in free_models:
            ...
            m_id = m["id"]
            # Ensure :free suffix
            if append_free and not m_id.startswith("openrouter/") and ":free" not in m_id:
                m_id = f"{m_id}:free"

            if "openrouter/free" in m["id"]:
                continue
            if as_primary and m_id == model_id:
                continue
            current_primary = get_current_model(config)
            if not as_primary and current_primary:
                cur_provider, _, cur_model = current_primary.partition("/")
                if m_id == cur_model:
                    continue

            fb_provider = m_id.split("/")[0]
            fallbacks.append({"provider": fb_provider, "model": m_id})

        if not as_primary:
            # Insert the requested model into fallbacks if not already there
            fb_provider, _, fb_model = model_id.partition("/")
            existing_models = [fb.get("model") for fb in fallbacks]
            if model_id not in existing_models:
                insert_pos = 1 if any(fb.get("model") == smart_router_id for fb in fallbacks) else 0
                fallbacks.insert(insert_pos, {"provider": fb_provider, "model": model_id})
...
def cmd_list(args):
    ...
    if not get_api_keys():
        ...
        print("Or add to ~/.hermes/.env: OPENROUTER_API_KEY=sk-or-...")
...
def cmd_switch(args):
    ...
    if update_model_config(
        ...
        if as_fallback:
            ...
            print("Success! Hermes config updated.")
        ...
        print("\nRun 'hermes gateway restart' for changes to take effect.")
    else:
        print("Error: Failed to update Hermes config.")
        sys.exit(1)
...
def cmd_fallbacks(args):
    ...
    smart_router_id = "openrouter/free"
    current_model_id = current.split("/", 1)[-1] if current and "/" in current else current
    if not current or current_model_id != smart_router_id:
        fallbacks.append({"provider": "openrouter", "model": smart_router_id})

    for m in models:
        m_id = m["id"]

        if current and m_id == current_model_id:
            continue
        if "openrouter/free" in m_id:
            continue
...
def rotate(force: bool = False, fallback_count: int = 5):
    ...
    config = load_config()
    current = get_current_model(config)
    # Extract API model ID from 'provider/model' format
    current_api = current.split("/", 1)[-1] if current and "/" in current else current

    if current_api and not force:
        print(f"Testing current primary: {current_api}")
        ok, err = _test_model(current_api)
        if ok:
    ...
    provider, _, model = new_primary.partition("/")
    config.setdefault("model", {})
    config["model"]["provider"] = provider
    config["model"]["default"] = model
    ...
    for fb_id in verified_fallbacks:
        ...
        fallbacks.append({"provider": fb_provider, "model": fb_id})
...
def main():
    parser = argparse.ArgumentParser(
        prog="freeride",
        description="FreeRide - Free AI for Hermes Agent. Manage free models from OpenRouter."
    )
